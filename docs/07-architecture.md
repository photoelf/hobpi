...
┌──────────────────────▼───────────────────────────────────┐
│  apps/api — Cloudflare Worker (ноль зависимостей)        │
│  • auth: HMAC-проверка initData (WebCrypto)              │
│  • game: доход, найм, постройки, прокачка                │
│  • battle: авторитетная симуляция боя (packages/engine)  │
│  • pvp: матчмейкинг, Elo, реплеи                         │
│  • D1 (SQLite) в проде / node:sqlite локально            │
└──────────────────────┬───────────────────────────────────┘
                       │
┌──────────────────────▼───────────────────────────────────┐
...
| **Cloudflare Workers + D1** | Не засыпают на бесплатном тарифе (в отличие от Render/Fly), а D1 — тот же SQLite, что и локально: `schema.sql` один на оба окружения. |
| **Своя маршрутизация на `Request`/`Response`** | Fastify на Workers не работает. Web-стандартный обработчик крутится в обоих рантаймах без единой строки дублирования, а у API стало **ноль рантайм-зависимостей**. |
| **Адаптер БД вместо прямых вызовов** | Один асинхронный интерфейс `Db`, две реализации: D1 и `node:sqlite`. Тесты и локальная разработка идут на настоящем SQLite без сети. |
...
├── apps/
│   ├── api/                    ← @hobpi/api
│   │   ├── schema.sql          схема, общая для D1 и node:sqlite
│   │   ├── wrangler.toml       конфигурация Worker
│   │   └── src/
│   │       ├── worker.ts       точка входа Cloudflare
│   │       ├── server.ts       точка входа Node (мост node:http)
│   │       ├── app.ts          роутер и обработка ошибок
│   │       ├── http.ts         Ctx,матчинг путей, rate limit
│   │       ├── db.ts           интерфейс Db + запросы
│   │       ├── adapters/       d1.ts, node.ts
│   │       ├── auth.ts         проверка initData на WebCrypto
...
Единственный источник схемы — `apps/api/schema.sql`; он применяется и к локальной базе,
и к D1, поэтому окружения не расходятся.
...
| Размер воркера | 135 КБ, 31 КБ gzip (лимит бесплатного тарифа — 3 МБ) |
| Холодный старт воркера | ~4 мс |
| D1 бесплатно | 5 ГБ, 5 млн чтений и 100 тыс. записей в сутки |
| Rate limit | 120 запросов/мин на игрока |
