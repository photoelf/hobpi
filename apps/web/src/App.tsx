...
export function App() {
  const { loading, registered, offline, unauthorized, toast, state, refresh } = useGame();
  ...
  if (unauthorized) {
    return (
      <div className="center">
        <div>
          <div style={{ fontSize: 40 }}>🚪</div>
          <h1 style={{ marginTop: 10 }}>Не по адресу</h1>
          <div className="dim small" style={{ margin: '8px 0 16px', maxWidth: 300 }}>
            Игра живёт внутри Telegram — снаружи сервер не понимает, кто ты.
            Открой её через бота <b>@bandy_spb_bot</b>.
          </div>
          <a className="btn primary" href="https://t.me/bandy_spb_bot" style={{ textDecoration: 'none' }}>
            Открыть в Telegram
          </a>
        </div>
      </div>
